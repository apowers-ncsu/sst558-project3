...
recipe <- recipe(Diabetes_binary ~ ., data = train) |>
  ...
  ### note! learned that using valid newname=oldname in the select statement caused errors for tuning
  #try using step_rename?
  step_select(
    Diabetes_binary,
    HighBP_high.blood.pressure,
    BMI,
    DiffWalk_walking.difficulty,
    HighChol_high.cholesterol,
    HeartDiseaseorAttack_heart.problems.CHD.MI,
    Veggies_vegetables,
    #GenHlth_poor,
    #GenHlth_fair,
    #GenHlth_good,
    ...
    PhysHlth,
    MentHlth#,
    #PhysActivity_physical.activity,
    #Smoker_smoker,
    #Sex_male
  ) |>
...
A pretty tree, though completely unreadable. The best tree_depth is about 8, of those tested for tuning.

And the log-loss value for this classification tree evaluated on the training dataset:
...
grid_randomforest <- grid_regular(
  mtry(range=c(1,length(recipe$var_info$role=="predictor"))-1),
  trees(),
  min_n(),
  levels = c(3, 3, 3)
  )
...
tune_randomforest <- temp |> select_best(metric="mn_log_loss")
workflow_randomforest <- workflow_randomforest |>
  finalize_workflow(tune_randomforest)

#fit on full training data
fit_randomforest <- workflow_final_randomforest |>
  fit(train)
...
Another unreadable display of a carefully-designed predictive model. The best tree_depth is about 8, of those tested for tuning.

And the log-loss value for this random forest model evaluated on the training dataset:
...
```{r}
tibble(
  c("classification tree","random forest"),
...
Our best performing model is _______! Now, we'll set up our API environment, beginning with fitting this to the entire dataset.

```{r}
library(plumber)
r <- plumb("API.r")
r$run(port = 1776)
#is hosted at http://localhost:1776


#workflow_***** |> fit(data) |> tidy()
```
